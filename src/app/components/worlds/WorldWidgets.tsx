...
const ServiceBadge: React.FC<{ name: string }> = ({ name }) => {
  ...
  if (def.icon) {
    return (
      <div title={name} style={{ width: S.serviceIconSize, height: S.serviceIconSize, borderRadius: R.iconLg, overflow: 'hidden', flexShrink: 0 }}>
        <img src={def.icon} alt={name} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
      </div>
    );
  }
  return (
        ...
        {WORLDS.map(w => {
          ...
          return (
            ...
            >
              ...
              {/* Row 2: subtitle */}
              <p style={{ color: C.textQuiet, fontSize: 14, lineHeight: 1.3, marginBottom: 14 }}>
                {w.sub}
              </p>

              {/* Row 3: insight */}
              <p style={{ color: C.textHighEmphasis, fontSize: 17, lineHeight: 1.3, marginBottom: 18 }}>
                {w.insight}
              </p>
