...
const CONTEXT_CHATS: Record<string, ContextChat> = {
  ...
  'home-office': { color: '#5E5CE6', message: 'Осень. Wildberries открыт чаще, чем обычно.\nМонитор, кресло, лампа — по одному. Домашний офис собирался неделями.' },

  // Memories
  'mem-kinopoisk':  { color: '#0A84FF', message: 'Семь фильмов. Пять дней. КиноПоиск считал — ты не спал.\nПоследний закончился в четыре утра. Рассвет застал с открытыми глазами.' },
  'mem-music':      { color: '#BF5AF2', message: 'Семьдесят два раза подряд. Потом — следующий.\nЯндекс Музыка помнит название трека. Ты уже, возможно, нет.' },
  'mem-food':       { color: '#FF9F0A', message: 'Пятница. Дождь. Приложение открыто раньше, чем начался голод.\nСуши или рамен — зависело от того, как прошла неделя.' },

  // Инциденты
  'incident-scooters': { color: '#FF3B30', message: 'Поездка началась. Самокат заблокировался раньше, чем ты доехал.\nМаршрут сохранился. Оплата — нет. Яндекс разбирается.' },

  // Инсайты — Спорт
  'ins-sports-f1':     { color: '#FF3B30', message: 'Прямой эфир — или не считается. Запись Саша не смотрит.\nГонка без спойлеров — это отдельная дисциплина.' },
  'ins-sports-nba':    { color: '#FF9F0A', message: 'Плей-офф. Активность — в три раза выше обычного.\nЭто уже не просто спорт. Это расписание.' },
  'ins-sports-stats':  { color: '#5AC8F5', message: 'Гонка закончилась — начинается разбор. Телеметрия, стратегия, шины.\nСмотреть мало. Важно понять почему.' },
  'ins-sports-time':   { color: '#BF5AF2', message: 'Будни, после десяти вечера. Когда всё остальное закончилось — включается спорт.\nНе фон. Полный просмотр.' },
  'ins-sports-tennis': { color: '#30D158', message: 'Теннис вошёл в топ-5 этого сезона. Тихо, без объявлений.\nАвстралийский открытый изменил цифры — надолго.' },

  // Инсайты — Музыка
  'ins-music':   { color: '#E03366', message: 'Электроника по пятницам после 22:00 — в два раза чаще, чем в любой другой день.\nНе случайность. Паттерн.' },
  'ins-music-2': { color: '#BF5AF2', message: 'Один альбом. Минимум три раза подряд. Иногда больше.\nЭто не рассеянность — так слушают то, что важно.' },
  'ins-music-3': { color: '#5E5CE6', message: 'С 23:00 до часа ночи. Пик стабильный — каждую неделю.\nНочная музыка — отдельный жанр. Один и тот же.' },
  'ins-music-4': { color: '#FF9F0A', message: 'Пост-рок вырос на сорок процентов за месяц. Постепенно, незаметно.\nTalk Talk. Godspeed. Потом что-то ещё.' },
  'ins-music-5': { color: '#30D158', message: 'Альбомы целиком. Не перемешивать, не обрезать.\nПлейлист — когда нужен фон. Это — не фон.' },

  // Инсайты — Кино
  'ins-kinopoisk': { color: '#FF6600', message: 'Аниме появилось три месяца назад. Сначала один тайтл, потом ещё.\nПрофиль изменился — тихо, без предупреждения.' },
  'ins-cinema-2':  { color: '#FF9F0A', message: 'Вишлист растёт в четыре раза быстрее, чем список просмотренного.\nОдин посмотрен — четыре добавлены вместо него.' },
  'ins-cinema-3':  { color: '#BF5AF2', message: 'Выходные, после десяти. Кино начинается когда всё остальное заканчивается.\nДнём — другие дела. Ночью — другие миры.' },
  'ins-cinema-4':  { color: '#FF3B30', message: 'Семьдесят два процента — исторические драмы. Почти три четверти.\nОстальное — случайное. Это — выбор.' },
  'ins-cinema-5':  { color: '#8E8E93', message: 'Второй эпизод. Дальше — редко. Либо зацепило сразу, либо нет.\nКиноПоиск видит это в каждом сезоне.' },

  // Инсайты — Покупки
  'ins-market':     { color: '#FFD60A', message: 'Воскресенье — главный день. Список накапливается за неделю.\nОткрывает приложение утром. Закрывает с заказом.' },
  'ins-split':      { color: '#30D158', message: 'Сплит — и платит в срок. Каждый раз.\nДоверие считается в числах. Числа растут.' },
  'ins-shopping-3': { color: '#FF6633', message: 'Восемь визитов в среднем — до того, как нажать «купить».\nИзучает. Сравнивает. Возвращается. Потом берёт.' },
  'ins-shopping-4': { color: '#5AC8F5', message: 'Четыре дня от первого поиска до покупки. Стабильно.\nНе импульс — решение, принятое заранее.' },
  'ins-shopping-5': { color: '#FF9F0A', message: 'Следующий день. Девяносто процентов заказов.\nЕсли можно быстро — зачем медленно.' },

  // Инсайты — Путешествия
  'ins-travel':   { color: '#00C7BE', message: 'Поздний вечер — время искать билеты. Не утром, не в обед.\nЯндекс видит запросы после одиннадцати. Стабильно.' },
  'ins-travel-2': { color: '#FF9F0A', message: 'Места сохраняются до поездки — в три раза активнее обычного.\nКарта заполняется. Маршрут формируется сам.' },
  'ins-travel-3': { color: '#5E5CE6', message: 'Две-три недели до вылета — стандарт. Не за день, не за полгода.\nЗаранее, но без паники.' },
  'ins-travel-4': { color: '#0A84FF', message: 'Европа — восемьдесят процентов всех поисков.\nАзия — остальное. Америка — почти нет.' },
  'ins-travel-5': { color: '#FF5A5F', message: 'Апартаменты — особенно на длинные поездки.\nОтель — на две ночи. Квартира — когда остаёшься жить.' },
};
